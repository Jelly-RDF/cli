...
class RdfInspectSpec extends AnyWordSpec with Matchers with TestFixtureHelper:
  ...
  "rdf inspect command" should {
    ...
    "print detailed metrics" when {
      "aggregating and flat" in withFullJellyFile(testCode = { j =>
      ...
      "per frame and flat" in withFullJellyFile(testCode = { j =>
        val (out, err) = RdfInspect.runTestCommand(List("rdf", "inspect", "--per-frame", "--detail", "flat", j))
        val yaml = new Yaml()
      ...
      "aggregating and grouping by node" in withFullJellyFile(testCode = { j =>
      ...
      "per frame and grouping by node" in withFullJellyFile(testCode = { j =>
        val (out, err) = RdfInspect.runTestCommand(List("rdf", "inspect", "--per-frame", "--detail", "node", j))
        val yaml = new Yaml()
        val parsed = yaml.load(out).asInstanceOf[util.Map[String, Any]]
        parsed.get("frames") shouldBe a[util.ArrayList[?]]
        val frames = parsed.get("frames").asInstanceOf[util.ArrayList[Any]]
      ...
      "aggregating and grouping by term" in withFullJellyFile(testCode = { j =>
      ...
      "per frame and grouping by term" in withFullJellyFile(testCode = { j =>
        val (out, err) = RdfInspect.runTestCommand(List("rdf", "inspect", "--per-frame", "--detail", "term", j))
        val yaml = new Yaml()
        val parsed = yaml.load(out).asInstanceOf[util.Map[String, Any]]
        parsed.get("frames") shouldBe a[util.ArrayList[?]]
        val frames = parsed.get("frames").asInstanceOf[util.ArrayList[Any]]
